...
export function ProduceForm({ model, materialsById, onConfirm, onClose }) {
  ...
  return (
    <Modal onClose={onClose} title={`บันทึกผลิต — ${model.name}`}>
      ...
      <div style={{ fontSize: 12, color: C.textDim, margin: '10px 0 6px' }}>วัตถุดิบที่จะถูกหักออกจากคลัง:</div>
        ...
        {model.bom.map(b => {
          ...
          return (
            <div key={b.material_id} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', fontSize: 12, borderBottom: `1px solid ${C.line}`, color: enough ? C.text : C.red }}>
              <span>{m.name}</span>
              <span style={mono}>-{b.qty} {m.unit} {!enough && '⚠'}</span>
            </div>
          );
        })}
      </div>
      <button onClick={submit} disabled={!canSubmit}
        ...
        <Save size={14} style={{ marginRight: 6 }} /> {busy ? 'กำลังบันทึก...' : 'ยืนยันบันทึก'}
      </button>
    </Modal>
  );
}
