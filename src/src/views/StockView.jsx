        ...
        <div>
          ...
          <div className="grid-list">
            {materials.filter(m => m.name.toLowerCase().includes(search.trim().toLowerCase())).map(m => (
              <div key={m.id} style={{ background: m.qty <= (m.low_stock_threshold ?? 2) ? 'rgba(217,119,87,0.08)' : C.panel, border: `1px solid ${C.line}`, borderRadius: 10, padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12.5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.name}</div>
                  <div style={{ fontSize: 11, marginTop: 2, ...mono, color: m.qty <= (m.low_stock_threshold ?? 2) ? C.red : C.teal, fontWeight: 700 }}>{m.qty} {m.unit}</div>
                </div>
