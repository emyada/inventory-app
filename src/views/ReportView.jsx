...
export function ReportView({ transactions, onCancel, sheetsWebhookUrl }) {
  ...
  // 4. โครงสร้างข้อมูลสำหรับ Export CSV
  const rowsSummaryExport = () => {
    const rows = [];
    CATEGORIES.forEach(cat => {
      if (byCategory[cat]) {
        Object.entries(byCategory[cat].models).forEach(([model, count]) => {
          rows.push({ หมวดหมู่: cat, รุ่นสินค้า: model, จำนวนที่ผลิต: count });
        });
      }
    });
    return rows;
  ...
  const rowsMaterialExport = () => materialList.map(m => ({
    ชื่อวัตถุดิบ: m.name, ยอดเบิกรวม: m.qty, หน่วย: m.unit
  }));
  ...
  async function handleSendToSheet() {
    ...
    setSendMsg(error || 'ส่งข้อมูลสรุปเข้า Google Sheet เรียบร้อยแล้ว');
    setSending(false);
  }
  ...
  return (
    <div style={{ paddingBottom: 20 }}>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
        <button onClick={() => downloadCSV(`summary_production_${from}_to_${to}.csv`, toCSV(rowsSummaryExport(), ['หมวดหมู่', 'รุ่นสินค้า', 'จำนวนที่ผลิต']))} style={{ ...btnGhost, flex: 1, justifyContent: 'center', fontSize: 11 }}>
          <Download size={12} style={{ marginRight: 4 }} /> โหลดสรุปสินค้า
        </button>
        <button onClick={() => downloadCSV(`summary_materials_${from}_to_${to}.csv`, toCSV(rowsMaterialExport(), ['ชื่อวัตถุดิบ', 'ยอดเบิกรวม', 'หน่วย']))} style={{ ...btnGhost, flex: 1, justifyContent: 'center', fontSize: 11 }}>
          <Package size={12} style={{ marginRight: 4 }} /> โหลดสรุปวัตถุดิบ
        </button>
        <button onClick={handleSendToSheet} disabled={sending} style={{ ...btnGhost, flex: 1, justifyContent: 'center', fontSize: 11 }}>
          <Send size={12} style={{ marginRight: 4 }} /> {sending ? 'กำลังส่ง...' : 'ส่ง Google Sheet'}
        </button>
      </div>
      {sendMsg && <div style={{ fontSize: 11.5, color: sendMsg.includes('เรียบร้อย') ? C.teal : C.red, marginBottom: 10 }}>{sendMsg}</div>}

      {/* 📊 TAB 1: DASHBOARD สรุปยอด */}
      {activeTab === 'dashboard' && (
        <div>
      ...
      {/* 📋 TAB 2: รายการดิบ (โครงสร้างเดิม + ปุ่มยกเลิก) */}
      {activeTab === 'raw' && (
        <div>
